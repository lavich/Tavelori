...
- [ ] 1.1 Тест в `tests/plan.test.ts`, рядом с блоком «темп принадлежит курсу»
...
- [ ] 1.4 Запустить тест — PASS

Команда: `npx vitest run tests/plan.test.ts -t 'карточки следующего занятия'`

- [ ] 1.5 Тест: сроки и темп по-прежнему считаются по всем предстоящим занятиям
...
- [ ] 1.6 Тест: добор бюджета не подбирает карточки дальнего занятия
...
- [ ] 1.7 Прогнать весь файл и закоммитить
...
- [ ] 2.1 Тест состава и порядка подготовки
...
- [ ] 2.2 Запустить — падает на отсутствии `plan.preview`

Команда: `npx vitest run tests/plan.test.ts -t 'досрочная подготовка'`
Ожидание: FAIL, TypeScript и рантайм на `plan.preview`

- [ ] 2.3 Вынести ранг состояния на уровень модуля в `src/domain/learning.ts`
...
- [ ] 2.4 Добавить `preview` в интерфейсы плана
...
- [ ] 2.5 Собрать подготовку в теле `makePlan`
...
- [ ] 3.1 Расширить режим в `src/domain/types.ts`
...
- [ ] 3.3 Добавить четвёртую категорию в `makeSession`
...
- [ ] 3.5 Тест: новые и повторения имеют приоритет над подготовкой
...
- [ ] 4.1 Тест верного ответа в подготовке
...
- [ ] 4.6 Тест: дополнительная попытка второй раз карточку не наказывает
...
- [ ] 4.7 Проверить, что режим больше нигде не читается
...
- [ ] 5.1 Добавить строку под плиткой повторения
...
- [ ] 6.1 Посмотреть, как изменится эталон, до перезаписи
