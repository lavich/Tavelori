...
export function diagnostics(error:unknown,reportId:string|null,at:Date=new Date()):string{
 ...
 const env=launch.kind==='telegram'?`Telegram${launch.platform?` ${launch.platform}`:''}${launch.version?` ${launch.version}`:''}`:'веб';
 const kind=error instanceof Error?error.name:typeof error;
 return [`Lexi ${APP_VERSION} (${APP_BUILD})`,`Среда: ${env}`,`Время: ${at.toISOString()}`,`Отчёт: ${reportId??'не отправлен'}`,`Ошибка: ${kind}`].join('\n');
}

interface Props {title:string;description:string;error:unknown;reportId:string|null;testId:string}
...
export function CrashScreen({title,description,error,reportId,testId}:Props){
 ...
 const copy=async()=>{
  ...
  catch{setCopied('manual')} // WebView без буфера обмена: текст показывается для ручного копирования
 };
 return (
