...
export function fromSnapshot(data:Snapshot):SessionSource&StatsSource{
 ...
 return {
  ...
  lessonRefs:async lessonId=>items.filter(item=>item.lessonId===lessonId).sort((a,b)=>a.position-b.position||a.unitKey.localeCompare(b.unitKey)).map(item=>item.ref),
  introducedTodayByCourse:async(today,timezone)=>{
   const counts=new Map<string,number>();
   for(const state of data.states){
    if(localDay(new Date(state.introducedAt),timezone)!==today)continue;
    const owners=new Set(items.filter(item=>item.unitKey===state.unitKey).map(item=>courseOfLesson.get(item.lessonId)??LOCAL_COURSE));
    if(!owners.size)owners.add(LOCAL_COURSE);
    for(const courseId of owners)counts.set(courseId,(counts.get(courseId)??0)+1);
   }
   return counts;
  },
  statesOf:async refs=>new Map(refs.map(unitKey).filter(key=>states.has(key)).map(key=>[key,states.get(key)!])),
  ...
  lessonBoundKeys:async refs=>{const bound=new Set(items.map(item=>item.unitKey));return new Set(refs.map(unitKey).filter(key=>bound.has(key)))},
  ...
  factsOf:async refs=>new Map(refs.filter(isLive).map(ref=>{
   const facts:CardFacts={kind:ref.kind};
   if(ref.kind==='phrase'){const phrase=phrases.get(ref.id)!;facts.hasTranslation=!!phrase.translation;facts.hasAudio=!!phrase.audioAssetId}
