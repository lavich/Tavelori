...
export function fromSnapshot(data: Snapshot): SessionSource & StatsSource {
  ...
  const record = (ref: LearningRef) =>
    ref.kind === "word" ? words.get(ref.id) : ref.kind === "phrase" ? phrases.get(ref.id) : clozes.get(ref.id);
  ...
  const cardOf = (ref: LearningRef): SessionCard | undefined => {
    ...
    return ref.kind === "word"
      ...
      : ref.kind === "phrase"
        ...
        : { kind: "cloze", cloze: clozes.get(ref.id)! };
  };
  const total = (kind: CardKind) =>
    kind === "word" ? data.words.length : kind === "phrase" ? phrases.size : clozes.size;
  return {
