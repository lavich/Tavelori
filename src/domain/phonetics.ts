...
export function coreWord(greek:string):string{
 ...
 return source.find(token=>[...token].some(char=>ACCENTED.includes(lower(char))))??source[source.length-1]??greek;
}
const nuclei=(word:string)=>{
 const text=lower(word), found:{start:number;length:number;accented:boolean}[]=[];
 for(let i=0;i<text.length;){
  const pair=text.slice(i,i+2);
  if(DIGRAPHS.includes(pair)){found.push({start:i,length:2,accented:[...pair].some(char=>ACCENTED.includes(char))});i+=2;continue}
  if(VOWELS.includes(text[i])){found.push({start:i,length:1,accented:ACCENTED.includes(text[i])});i+=1;continue}
  i+=1;
 }
 return found;
};
