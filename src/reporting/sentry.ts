...
function init({dsn,release,tags}:SdkOptions){
 Sentry.init({
  dsn,release,environment:tags.env,
  sendDefaultPii:false,
  sampleRate:1,
  maxBreadcrumbs:30,
  initialScope:{tags},
  transport:Sentry.makeBrowserOfflineTransport(Sentry.makeFetchTransport),
  transportOptions:{dbName:OFFLINE_DB,storeName:'queue',maxQueueSize:30,flushAtStartup:true} as Sentry.BrowserOptions['transportOptions'],
  integrations:defaults=>[
   ...defaults.filter(integration=>integration.name!=='BrowserSession'&&integration.name!=='Breadcrumbs'),
   Sentry.breadcrumbsIntegration({console:false,dom:false,fetch:true,xhr:true,history:true,sentry:false}),
  ],
