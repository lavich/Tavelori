...
export const dominates=(a:Clock,b:Clock)=>Object.entries(b).every(([device,count])=>(a[device]??0)>=count);
...
export const mergeClocks=(...clocks:Clock[]):Clock=>{
 const merged:Clock={};
 for(const clock of clocks)for(const [device,count] of Object.entries(clock))merged[device]=Math.max(merged[device]??0,count);
 return merged;
};
