 ...
 * Формат 2 — типизированные ссылки на карточки трёх видов и код проверки `cloze`; формат 1 читается как словарный.
 */
...
export interface CompactState {ref:LearningRef;card:SerializedCard;introducedAt:string;version:number}
...
export interface CompactCourse {id:string;subscribed:boolean;newItemsPerDay:number;schedule:Schedule}
/**
...
export interface BaseSkillRow {unitKey:string;ref:LearningRef;skills:SkillSummary}
...
export interface StashRow {unitKey:string;ref:LearningRef;state:CompactState}
