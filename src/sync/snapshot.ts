...
import {LEGACY_SNAPSHOT_FORMAT, SNAPSHOT_FORMAT, type Clock, type CompactLesson, type CompactSnapshot, type CompactState, type SerializedCard} from './types';
...
const serializeState=(state:LearningState):CompactState=>({ref:state.ref,card:serializeCard(state.card),introducedAt:state.introducedAt,version:state.version});
const reviveState=(state:CompactState):LearningState=>({unitKey:unitKey(state.ref),ref:state.ref,card:reviveCard(state.card),introducedAt:state.introducedAt,version:state.version});
...
export async function buildSnapshot(database:LexiDatabase,now:Date):Promise<CompactSnapshot>{
 ...
 const known=await standardKeys(database,rawStates.map(state=>state.ref));
 ...
 const skills=new Map<string,{ref:LearningRef;skills:SkillSummary}>((await database.cardSkills.toArray()).map(row=>[row.unitKey,{ref:row.ref,skills:row.skills}]));
 const eventKeys=await standardKeys(database,uniqueRefs(fresh.map(event=>event.ref)));
 for(const event of fresh)if(eventKeys.has(event.unitKey))skills.set(event.unitKey,{ref:event.ref,skills:foldSkill(skills.get(event.unitKey)?.skills??emptySkills(),event)});
 ...
 return {
  ...
  courses:(await database.courses.toArray()).map(course=>({id:course.id,subscribed:course.subscribed,newItemsPerDay:course.newItemsPerDay,schedule:course.schedule})).sort((a,b)=>a.id.localeCompare(b.id)),
...
export async function hasMixedProgress(database:LexiDatabase):Promise<boolean>{
 ...
 const base=await database.baseSummary.get('base');
 if(base?.stats.answeredKeys.some(key=>!key.startsWith('["word"'))||base?.stats.days.some(day=>day.keys.some(key=>!key.startsWith('["word"'))))return true;
...
export async function applySnapshot(database:LexiDatabase,snapshot:CompactSnapshot,versionId:string,clock:Clock,now:Date,sourceFormat:number=SNAPSHOT_FORMAT):Promise<void>{
 await database.transaction('rw',SNAPSHOT_TABLES.map(name=>database.table(name)),async()=>{
  ...
  for(const incoming of snapshot.courses){
   ...
   if(stored)await database.courses.put({...stored,subscribed:incoming.subscribed,newItemsPerDay:incoming.newItemsPerDay,schedule:incoming.schedule});
  }
  ...
  const localStandard=await standardKeys(database,local.map(state=>state.ref));
  // Стандартные карточки, которых нет в выбранной версии, снова становятся новыми; пользовательские не трогаем.
  await database.cardStates.bulkDelete(local.filter(state=>localStandard.has(state.unitKey)&&!incoming.has(state.unitKey)).map(state=>state.unitKey));
  const known=await standardKeys(database,snapshot.states.map(state=>state.ref));
...
export async function adoptStash(database:LexiDatabase,lessonId:string,refs:LearningRef[]):Promise<void>{
 const rows=(await database.cardStash.bulkGet(refs.map(unitKey))).filter((row):row is NonNullable<typeof row>=>!!row);
...
export interface SnapshotDescription {cards:number;answers:number;lastDay:string|null;lessons:number}
export const describeSnapshot=(snapshot:CompactSnapshot):SnapshotDescription=>({
 cards:snapshot.states.length,answers:snapshot.stats.answers,lessons:snapshot.lessons.length,
...
export const hasLocalProgress=async(database:LexiDatabase)=>(await database.cardStates.count())>0||(await database.events.count())>0;
