...
export interface SyncConflict {kind:'diverged'|'initial'|'restored'|'remote';branches:ConflictBranch[]}
...
export class SyncCoordinator {
 ...
 get adapter(){return this.options.adapter}
 setAdapter(adapter:SyncAdapter){this.options.adapter=adapter;this.update({error:null,phase:adapter.capabilities().available?'idle':'disabled'})}
 subscribe=(listener:()=>void)=>{this.listeners.add(listener);return()=>{this.listeners.delete(listener)}};
 getStatus=()=>this.status;
 private update(patch:Partial<SyncStatus>){this.status={...this.status,...patch};this.listeners.forEach(listener=>listener())}
 ...
 private async decide(writes:boolean,depth:number):Promise<void>{
  ...
  candidates=candidates.filter(candidate=>!candidates.some(other=>other!==candidate&&(other.resolves.includes(candidate.id)||(dominates(other.clock,candidate.clock)&&!sameClock(other.clock,candidate.clock)))));
  ...
  if(!candidates.length){
   ...
   if(flags.restored&&listing.pointers.length){
    ...
    for(const pointer of latest){
     ...
     remote.push({id:pointer.id,device:pointer.device,label:pointer.device===device?'Это устройство до восстановления':pointer.label??pointer.device,createdAt:pointer.createdAt,local:false,description:describeSnapshot(snapshot),snapshot,meta:pointer});
    }
    const snapshot=await database.transaction('r',SNAPSHOT_TABLES.map(name=>database.table(name)),()=>buildSnapshot(database,this.options.now()));
    this.conflict('restored',[{id:`local-${device}`,device,label:'Восстановленная копия',createdAt:snapshot.createdAt,local:true,description:describeSnapshot(snapshot),snapshot,meta:null},...remote]);
    return;
   }
   if(flags.dirty||flags.restored||(never&&local&&!mine)){
    if(!writes){this.update({phase:'paused',reason:'Браузер не поддерживает блокировки вкладок: облачная запись приостановлена, данные сохранены на устройстве.'});return}
  ...
  for(const candidate of candidates){
   try{
    ...
    remote.push({id:candidate.id,device:candidate.device,label:candidate.label??candidate.device,createdAt:candidate.createdAt,local:false,description:describeSnapshot(snapshot),snapshot,meta:candidate});
   }catch(error){
    // Поколение убрали во время чтения: перечитываем указатели, а не применяем частичные данные.
    if(error instanceof SyncError&&error.kind==='integrity'&&depth<1)return this.decide(writes,depth+1);
    throw error;
   }
  }
  const localBranch=async():Promise<ConflictBranch>=>{
   const snapshot=await database.transaction('r',SNAPSHOT_TABLES.map(name=>database.table(name)),()=>buildSnapshot(database,this.options.now()));
   return {id:`local-${device}`,device,label:'Это устройство',createdAt:snapshot.createdAt,local:true,description:describeSnapshot(snapshot),snapshot,meta:null};
  };
  if(flags.restored){this.conflict('restored',[await localBranch(),...remote]);return}
  if(never&&local){this.conflict('initial',[await localBranch(),...remote]);return}
  if(!flags.dirty){
   if(remote.length===1){
    const [branch]=remote;
 ...
 private async publish(device:string,base:Clock,mergeWith:Clock[],resolves:string[],override:CompactSnapshot|null,mine:VersionMeta|null){
  const {adapter,database}=this.options;
  ...
  await adapter.publishVersion({id,device,clock,createdAt:now.toISOString(),format:SNAPSHOT_FORMAT,resolves,label:this.options.label||undefined},snapshot);
 ...
 private async cleanup(device:string,currentId:string,_previous:VersionMeta|null){
  ...
  try{
   ...
   const own=(await adapter.listGenerations()).filter(id=>id.startsWith(`${device}-`)&&id!==currentId&&!protectedIds.has(id));
  ...
  }catch(error){console.warn('Очистка старых поколений отложена',error)}
 }
 private async installMissing(packages:string[]){
  ...
  }catch{/* установка пакетов — забота экрана уроков */}
 }
 ...
 */
 async resolve(branchId:string):Promise<SyncStatus>{
  ...
  await database.syncVersions.bulkPut(rejected.map((branch):SyncVersionRow=>({
   id:branch.id,role:'rejected',createdAt:now,
   meta:branch.meta??{id:branch.id,device,clock:{},createdAt:branch.createdAt,format:SNAPSHOT_FORMAT,parts:0,checksum:'',resolves:[]},
   snapshot:branch.snapshot,note:`Отвергнута при выборе версии ${chosen.label}`,
  })));
 ...
 async discardStored(id:string){await this.options.database.syncVersions.delete(id)}
 ...
 async exportStored(id:string):Promise<Blob|null>{
  ...
  return row?new Blob([JSON.stringify({kind:'lexi-sync-version',meta:row.meta,snapshot:row.snapshot})],{type:'application/json'}):null;
 }

 /** Автоматические поводы обмена; ручной повтор — `exchange()`. */
 start(){
  this.stop();
  const visible=()=>{if(document.visibilityState==='visible')void this.exchange()};
  ...
  this.stopHandlers=[()=>document.removeEventListener('visibilitychange',visible),()=>window.removeEventListener('online',online),off,()=>pending?.()];
  void this.exchange();
 }
 stop(){this.stopHandlers.forEach(handler=>handler());this.stopHandlers=[];this.cancelRetry?.();this.cancelRetry=null}
}

/** Web Locks: одна вкладка пишет, остальные пропускают; без API — запись приостанавливается. */
export const webLock=(name:string)=>async(run:()=>Promise<void>):Promise<LockResult>=>{
 if(typeof navigator==='undefined'||!navigator.locks)return 'unsupported';
 let result:LockResult='busy';
 await navigator.locks.request(name,{ifAvailable:true},async lock=>{if(lock){result='acquired';await run()}});
 return result;
};
