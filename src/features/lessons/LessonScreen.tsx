...
const GROUPS:{kind:CardKind;title:string;forms:[string,string,string]}[]=[
 {kind:'word',title:'Слова',forms:WORDS},{kind:'phrase',title:'Фразы',forms:PHRASES},
];
   ...
   <Screen back={entry.title}>
    <Card className="mb-3 bg-soft ring-0">
     <CardHeader>
      ...
      <CardTitle className="text-xl font-semibold">{compositionLabel({word:entry.wordCount,phrase:entry.phraseCount})}</CardTitle>
   ...
   {GROUPS.filter(group=>counts[group.kind]>0).map(group=>(
    <section key={group.kind} data-testid={`group-${group.kind}`}>
     <div className="mt-4 mb-2 flex items-center justify-between gap-2">
      ...
      {counts.phrase>0&&<Button size="sm" variant="quiet" className="w-auto" onClick={()=>practice(group.kind)}>Потренировать группу</Button>}
     </div>
