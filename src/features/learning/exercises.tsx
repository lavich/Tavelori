...
function ClozeIntro({ cloze }: { cloze: Cloze }) {
  ...
  return (
    <>
      <div className={cx(ui.row, ui.between)} style={{ width: "100%", gap: 12 }}>
        <div className={ui.grow} style={{ minWidth: 0, textAlign: "left" }}>
          <p className={wordCss.greek} style={{ margin: 0 }} data-testid="cloze-example">
            {before}
            <span className={wordCss.target}>{cloze.answer}</span>
            {after}
          </p>
          {cloze.context && <p style={{ fontSize: 19, margin: "6px 0 0" }}>{cloze.context}</p>}
        </div>
        <SpeakText
          text={fillGap(cloze.template, cloze.answer)}
          audioAssetId={cloze.audioAssetId}
          label="Послушать предложение"
        />
      </div>
      {cloze.explanation && (
        <p className={ui.small} style={{ width: "100%", textAlign: "left", margin: 0 }}>
          {cloze.explanation}
        </p>
      )}
    </>
...
export function Introduction({
  ...
  const title =
    card.kind === "word" ? "Новое слово" : card.kind === "phrase" ? "Новая фраза" : "Новое задание с пропуском";
  return (
    <>
      ...
      <div className={s.center}>
        ...
        {!result && (
          <p className={wordCss.greek} style={{ margin: "6px 0" }} data-testid="cloze-template">
            {before}
            <span aria-label="пропуск" className={s.gap}>
              …
            </span>
            {after}
          </p>
        )}
        {cloze.context && !result && (
          <p style={{ fontSize: 19, margin: 0 }} data-testid="cloze-context">
            {cloze.context}
          </p>
        )}
        {result && (
          <div style={{ width: "100%" }} ref={revealed}>
            ...
            >
              <div>{result.message}</div>
              <p className="m-0 mt-1.5 text-[19px]" data-testid="cloze-sentence">
                {before}
                <span className={wordCss.target}>{result.expected}</span>
                {after}
              </p>
              {result.status !== "correct" && !result.skipped && (
                <p className={s.chars} data-testid="chars" style={{ margin: "6px 0 0" }}>
                  {diffChars(value, result.expected).map((part, index) =>
                    part.type === "same" ? (
                      <b key={index}>{part.text}</b>
                    ) : part.type === "wrong" ? (
                      <s key={index}>{part.text}</s>
              ...
              {cloze.explanation && (
                <p className={ui.small} style={{ margin: "6px 0 0" }} data-testid="cloze-explanation">
                  {cloze.explanation}
                </p>
              )}
            </div>
            <div className="mt-3 flex justify-center">
              <SpeakText text={sentence} audioAssetId={cloze.audioAssetId} label="Послушать предложение" />
            </div>
          </div>
        )}
      </div>
            ...
            <div className={s.options} style={{ width: "100%" }}>
              {choices.map((option) => (
                <Button
                  ...
                  variant="outline"
                  disabled={saving}
                  className="h-14 justify-center rounded-[14px] text-[17px]"
          ...
          <form onSubmit={submit}>
            <div className={s.field}>
              ...
              <input
                ...
                autoCorrect="off"
                spellCheck={false}
                aria-label="Пропущенная часть предложения"
                lang="el"
