...
export function WordEditorScreen(){
 ...
 if(!draft)return <><BackBar title="Редактор"/><main className="screen"><p className="muted">Слово не найдено.</p></main></>;
  ...
  <>
   ...
   <main className="screen">
    <form onSubmit={submit}>
     ...
     <p className="small muted">{draft.verified?'Фонетика проверена при подготовке исходного набора.':'Ваша запись хранится как пользовательская и не считается проверенной.'}</p>
     ...
     {draft.examples.map((example,index)=>(
      <div className="card" key={index}>
       ...
       <button type="button" className="btn quiet" style={{marginTop:10}}
        onClick={()=>patch({examples:draft.examples.filter((_,i)=>i!==index)})}>Удалить пример</button>
      </div>
     ))}
     <button type="button" className="btn ghost" onClick={()=>patch({examples:[...draft.examples,{...emptyExample}]})}>Добавить пример</button>
     ...
     <p className="small muted">{draft.audioAssetId?'Аудиофайл сохранён в базе.':'Файла нет — слово озвучивается системным греческим голосом, если он доступен.'}</p>

     {problem&&<p className="error" role="alert">{problem}</p>}
     <button className="btn" type="submit" style={{marginTop:16}}>Сохранить</button>
     {saved&&<p className="small" style={{color:'var(--ok)'}} role="status">Сохранено.</p>}
    </form>
    <button className="btn quiet" style={{marginTop:24}}
