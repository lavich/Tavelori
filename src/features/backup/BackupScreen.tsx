...
export function BackupScreen(){
 ...
 const restore=()=>{
  ...
  return run(async()=>{
   ...
   if(!handedOver(guard))return setProblem(`Замена отменена: защитная копия не передана (${TRANSFER_TEXT[guard].toLowerCase()})`);
   await restoreBackup(file);
   setStatus('Данные восстановлены полностью.');
  },error=>error instanceof Error?`${error.message} Текущие данные остались без изменений.`:'Восстановление не удалось, данные не изменены.');
 };
 return (
  <Screen back="Копия данных">
   <Card className="mb-3"><CardHeader><CardTitle>Полная копия</CardTitle></CardHeader><CardContent>
    <p className={ui.note}>Слова, наборы и их связи, скачанные картинки и аудио, версии установленных уроков и ваши правки, прогресс FSRS, ответы, сессии и настройки. Этот файл переносит всё{profile.kind==='telegram'?', в том числе между Telegram и обычным браузером':''}.</p>
    {profile.kind==='telegram'&&<p className={ui.note} data-testid="sync-boundaries">{SYNC_BOUNDARIES}</p>}
    <p className={ui.note} data-testid="error-reports-boundaries">{REPORT_BOUNDARIES}</p>
    <Button size="xl" onClick={()=>run(async()=>{await send(await exportFull(),backupName())})} disabled={busy}>Сохранить полную копию</Button>
   ...
   <Card className="mb-3"><CardHeader><CardTitle>Только слова (TSV)</CardTitle></CardHeader><CardContent>
    <p className={ui.note}>Греческий, перевод и IPA для переноса в другие приложения. Прогресс обучения в этот файл не входит.</p>
   ...
   {stored.length>0&&(
    <Card className="mb-3" data-testid="stored-versions"><CardHeader><CardTitle>Отложенные версии облака</CardTitle></CardHeader><CardContent>
     <p className={ui.note}>Версии, отвергнутые при выборе в конфликте. Хранятся на устройстве до удаления вручную; сначала можно сохранить файл.</p>
