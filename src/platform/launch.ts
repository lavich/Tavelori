...
export function launchContext(): LaunchContext {
  ...
  if (typeof window === "undefined")
    return (cached = { kind: "web", bot: DEFAULT_BOT, platform: null, version: null, user: null, startParam: null });
  ...
  try {
    ...
    if (stored) {
      const parsed = JSON.parse(stored) as LaunchContext;
      if (parsed.kind === "telegram") return (cached = { ...parsed, bot: fresh.bot });
    }
  } catch {
    /* нет хранилища */
  }
