...
export function loadTelegramBridge(timeoutMs=4000,doc:Document=document):Promise<TelegramWebApp|null>{
 ...
 loading=new Promise(resolve=>{
  ...
  script.src=BRIDGE_URL; script.async=true;
  const finish=(value:TelegramWebApp|null)=>{clearTimeout(timer);resolve(value)};
  const timer=setTimeout(()=>finish(null),timeoutMs);
  script.onload=()=>finish(window.Telegram?.WebApp&&typeof window.Telegram.WebApp.onEvent==='function'?window.Telegram.WebApp:null);
...
/** Только для тестов. */
export const resetBridge=()=>{loading=null};
