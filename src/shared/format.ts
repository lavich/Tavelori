const DATIVE:Record<string,string>={'понедельник':'понедельнику','вторник':'вторнику','среда':'среде','четверг':'четвергу','пятница':'пятнице','суббота':'субботе','воскресенье':'воскресенью'};
...
export const dayMonth=(value:string)=>day(value).toLocaleDateString('ru-RU',{day:'numeric',month:'long',timeZone:'UTC'});
export const capitalize=(text:string)=>text.charAt(0).toUpperCase()+text.slice(1);
export const shortTitle=(title:string)=>title.replace(/^Урок\s+/i,'');
export const minutes=(ms:number)=>`${Math.max(1,Math.round(ms/60000))} ${plural(Math.max(1,Math.round(ms/60000)),['минута','минуты','минут'])}`;
