...
export interface LegacyState {wordId:string;card:LearningState['card'];introducedAt:string;version:number}
export interface LegacyStash {wordId:string;state:{wordId:string;card:unknown;introducedAt:string;version:number}}
...
export class LexiDatabase extends Dexie {
 ...
 cardStates!:Table<LearningState,string>; events!:Table<ReviewEvent,string>; sessions!:Table<Session,string>;
 settings!:Table<Settings,string>; meta!:Table<MetaRow,string>;
 cardSkills!:Table<BaseSkillRow,string>; baseSummary!:Table<BaseSummaryRow,string>; syncVersions!:Table<SyncVersionRow,string>; cardStash!:Table<StashRow,string>;
 /** Хранилища до схемы 6 с первичным ключом `wordId`: после миграции пусты. */
 lessonWords!:Table<LessonWord,[string,string]>; states!:Table<LegacyState,string>; baseSkills!:Table<LegacySkill,string>; syncStash!:Table<LegacyStash,string>;
...
const localCourse=(now:string):Course=>({id:LOCAL_COURSE,title:'Мои слова',origin:'local',subscribed:true,schedule:defaultSchedule,newItemsPerDay:DEFAULT_NEW_ITEMS_PER_DAY,createdAt:now,updatedAt:now});
...
export async function migrateCards(tx:Pick<Transaction,'table'>):Promise<void>{
 ...
 for(const state of await states.toArray()){
  const ref=wordRef(state.wordId), key=unitKey(ref);
  if(!await cardStates.get(key))await cardStates.put({unitKey:key,ref,card:state.card,introducedAt:state.introducedAt,version:state.version});
 }
 await states.clear();
 const links=tx.table('lessonWords') as Table<LessonWord,[string,string]>;
 const items=tx.table('lessonItems') as Table<LessonItem,[string,string]>;
 for(const link of await links.toArray()){
  const ref=wordRef(link.wordId), key=unitKey(ref);
  if(!await items.get([link.lessonId,key]))await items.put({lessonId:link.lessonId,unitKey:key,ref,position:link.position});
 }
 await links.clear();
 ...
 for(const row of await skills.toArray()){
  ...
  if(!await cardSkills.get(key))await cardSkills.put({unitKey:key,ref:wordRef(row.wordId),skills:row.skills});
 }
 await skills.clear();
 ...
 for(const row of await stash.toArray()){
  const ref=wordRef(row.wordId), key=unitKey(ref);
  const {wordId:_wordId,...rest}=row.state;
  if(!await cardStash.get(key))await cardStash.put({unitKey:key,ref,state:{ref,card:rest.card as StashRow['state']['card'],introducedAt:rest.introducedAt,version:rest.version}});
 }
 await stash.clear();
 ...
 await events.toCollection().modify(event=>{
  ...
  event.ref=ref; event.unitKey=unitKey(ref);
  delete event.wordId;
 });
 ...
 await sessions.toCollection().modify(session=>{
  ...
  if(session.introducedWordIds){
   session.introducedKeys=[...new Set([...(session.introducedKeys??[]),...session.introducedWordIds.map(id=>unitKey(wordRef(id)))])];
 ...
 await summary.toCollection().modify(row=>{row.stats=migrateStats(row.stats)});
 const versions=tx.table('syncVersions') as Table<{id:string;snapshot:LegacySnapshot},string>;
 await versions.toCollection().modify(row=>{row.snapshot=migrateSnapshot(row.snapshot)});
}
type LegacyStats={days:{date:string;answers:number;wordIds?:string[];keys?:string[]}[];answeredWordIds?:string[];answeredKeys?:string[];[key:string]:unknown};
type LegacySnapshot={states?:{wordId?:string;ref?:unknown;[key:string]:unknown}[];skills?:{wordId?:string;ref?:unknown;[key:string]:unknown}[];stats?:LegacyStats;courses?:{newWordsPerDay?:number;newItemsPerDay?:number;[key:string]:unknown}[];[key:string]:unknown};
const migrateStats=({answeredWordIds,answeredKeys,days,...rest}:LegacyStats):LegacyStats=>({
 ...rest,
 days:(days??[]).map(({wordIds,keys,...day})=>({...day,keys:keys??(wordIds??[]).map(id=>unitKey(wordRef(id)))})),
...
const migrateSnapshot=(snapshot:LegacySnapshot):LegacySnapshot=>({
 ...snapshot,
 states:(snapshot.states??[]).map(({wordId,...rest})=>rest.ref?rest:{...rest,ref:wordRef(wordId??'')}),
 skills:(snapshot.skills??[]).map(({wordId,...rest})=>rest.ref?rest:{...rest,ref:wordRef(wordId??'')}),
 stats:snapshot.stats?migrateStats(snapshot.stats):snapshot.stats,
 courses:(snapshot.courses??[]).map(({newWordsPerDay,...rest})=>({...rest,newItemsPerDay:rest.newItemsPerDay??newWordsPerDay??DEFAULT_NEW_ITEMS_PER_DAY})),
});
