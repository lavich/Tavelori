...
export const RECOVERY_ATTEMPTS=3, RECOVERY_WINDOW_MS=60000;
...
export interface HealerOptions {reopen?:()=>Promise<void>;onRecovered?:(attempt:number)=>void;now?:()=>number}
/**
...
export function storageHealer(options:HealerOptions={}):(error:unknown)=>Promise<boolean>{
 ...
 return async error=>{
  ...
  try{await reopen()}
  catch(failure){console.warn('Не удалось переоткрыть локальную базу',failure);return false}
  onRecovered?.(attempt);
  return true;
 };
}
