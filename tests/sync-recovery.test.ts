...
function coordinator(database:LexiDatabase,recover:(error:unknown)=>Promise<boolean>){
 ...
 const sync=new SyncCoordinator({
  database,adapter:kvAdapter(cloud),now:()=>new Date(clockMs),label:'ios',
  schedule:()=>()=>undefined,retryBaseMs:1,recover,
 });
 sync.onFailure=(_error,kind)=>{failures.push(kind)};
 return {sync,failures};
}

beforeEach(()=>{cloud=memoryTransport()});
...
describe('отказ хранилища в синхронизации',()=>{
 it('лечится переоткрытием базы: обмен доходит до конца, отчёта нет, остаётся крошка с номером попытки',async()=>{
  ...
  const heal=storageHealer({reopen:()=>reopenDatabase(database),onRecovered:attempt=>attempts.push(attempt)});
