...
import {lessonLinks, searchWordIds} from '../src/storage/queries';
...
describe('миграция схемы без сети',()=>{
 it('переносит wordIds в связи с порядком, отмечает исходные уроки установленными и не трогает данные пользователя',async()=>{
  ...
  expect(await db.lessonWords.where('wordId').equals('w12-16').count()).toBe(3);
  ...
  expect((await db.states.get('w12-16'))!.version).toBe(2);
 ...
 it('после миграции первое обновление пакета заменяет нетронутые слова и сохраняет правки, удаления и порядок',async()=>{
  ...
  expect(result.conflicts.map(c=>c.wordId).sort()).toEqual(['w12-01','w12-02']); // правка и удаление против нового содержимого
  ...
  expect((await db.states.get('w12-16'))!.version).toBe(2);
  expect((await lessonLinks('lesson-1-2',db)).map(link=>link.wordId)).toEqual(wordsOf('lesson-1-2').map(w=>w.id));
...
describe('резервная копия',()=>{
 ...
 it('новая копия содержит связи, медиа и метаданные пакетов, но не каталог; восстановление воспроизводит данные',async()=>{
  ...
  expect(names).toEqual(expect.arrayContaining(['lessonWords','packages','media','words']));
  ...
  expect(await fresh.lessonWords.count()).toBe(30);
