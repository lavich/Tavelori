...
describe("оценка объективного ответа", () => {
  ...
  it("у заданий без вариантов время не читается", () => {
    for (const type of ["assembly", "spelling", "cloze"] as const)
      expect(gradeFor("correct", type, 1)).toBe(Rating.Good);
  });
});
...
describe("упражнения для фраз и пропусков", () => {
  ...
  it("пропуск всегда проверяется вводом текста без вариантов", () => {
    ...
    expect(exerciseFor({ kind: "cloze", cloze }, { words: [], phrases: [] }, emptySkills(), () => 0.5, true)).toEqual({
      type: "cloze",
      options: [],
    });
    expect(
      exerciseFor(
        { kind: "cloze", cloze },
        { words: [], phrases: [] },
        skills({ cloze: [false, false] }),
        () => 0.5,
        false,
      ),
    ).toEqual({ type: "cloze", options: [] });
  });
  it("единственная новая карточка любого вида идёт после доступных проверок; знакомство сохраняет основные места", async () => {
    ...
    const session = await sessionOf({
      source: fromSnapshot(snapshot({ ...data, words: words.slice(0, 2), clozes: [cloze], states: [state] })),
      now,
      refs: [{ kind: "cloze", id: "c" }, wordRef(words[1].id)],
    });
