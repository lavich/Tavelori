...
const lesson=(id:string,courseId:string|undefined,wordCount=10,over:Partial<LessonView>={}):LessonView=>({id,courseId,title:id,targetDate:null,status:'upcoming',createdAt:iso,updatedAt:iso,wordCount,cardCount:wordCount,phraseCount:0,...over});
const entry=(id:string,courseId:string):CatalogEntry=>({id,courseId,language:'el',title:id,wordCount:5,phraseCount:0,cardCount:5,version:'v1',url:`content/packages/${id}@v1.json`,bytes:100,media:{count:0,bytes:0}});
...
describe('ближайшее занятие в списке уроков',()=>{
 ...
 const dated=(id:string,courseId:string,targetDate:string|null,over:Partial<LessonView>={})=>lesson(id,courseId,10,{targetDate,...over});
