...
describe('деление на слоги',()=>{
 ...
 /** «το φρούτο» кончается на собственный артикль: чистка по совпадению строки съедала бы последний слог. */
 it('слог, совпавший с артиклем, из набора не пропадает',()=>{
  expect(tiles('το φρούτο')).toEqual(['φρού','το']);
  expect(assemblyOptions('το φρούτο',['το','φρού','το'])).toEqual(['φρού','το']); // старая сессия: артикль лишний
  expect(assemblyOptions('το φρούτο',['φρού','το'])).toEqual(['φρού','το']);      // новая сессия: чистить нечего
  expect(assemblyOptions('το φρούτο',['το','φρού'])).toEqual(['το','φρού']);      // столько же плиток, сколько слогов — не трогаем
 });
});
