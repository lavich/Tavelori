...
class FakeUtterance {
 text:string; voice:unknown=null; lang=''; rate=1;
 onstart:(()=>void)|null=null; onend:(()=>void)|null=null; onerror:((event:{error:string})=>void)|null=null;
 constructor(text:string){this.text=text}
}
interface FakeState {speaking:boolean;pending:boolean;spoken:FakeUtterance[];cancels:number;script:Behavior[];voices:{lang:string;name:string}[]}
function fakeSynth(over:Partial<FakeState>={}){
 const state:FakeState={speaking:false,pending:false,spoken:[],cancels:0,script:[],voices:[{lang:'el-GR',name:'Greek test'}],...over};
 ...
 const synth={
  ...
  speak(utterance:FakeUtterance){
   ...
   if(behavior==='ok'){state.speaking=true;setTimeout(()=>{utterance.onstart?.()},1)}
  ...
  removeEventListener(type:string,listener:()=>void){listeners.set(type,(listeners.get(type)??[]).filter(item=>item!==listener))},
 };
 const fire=(type:string)=>[...(listeners.get(type)??[])].forEach(listener=>listener());
 return {synth,state,fire};
}
const word=(over:Partial<Word>={}):Word=>({id:'w1',greek:'το σπίτι',russian:'дом',ipa:'',segments:[],examples:[],verified:false,createdAt:'',updatedAt:'',...over});
...
describe('озвучка системным голосом',()=>{
 ...
 it('греческий голос выбирается среди чужих',async()=>{
  const {synth,state}=fakeSynth({voices:[{lang:'ru-RU',name:'Русский'},{lang:'el-GR',name:'Ελληνικά'}]});
  const {playWord}=await load(synth);
  const result=playWord(word());
  await vi.advanceTimersByTimeAsync(50);
  expect(await result).toBe('voice');
