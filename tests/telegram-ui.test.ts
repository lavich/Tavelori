...
let db:LexiDatabase;
beforeEach(async()=>{await new LexiDatabase('lexi-tg-ui').delete();db=new LexiDatabase('lexi-tg-ui');await db.open();await installLessons(db,['lesson-1-1'])});
...
describe('жизненный цикл ответа',()=>{
 it('повторная запись того же ответа сообщает created:false — отклик и синхронизация не повторяются',async()=>{
  ...
  const first=await recordAnswer({session,item,correct:true,answer:'',responseTimeMs:1,activeTimeMs:1,timezone:'UTC',now,database:db});
  const second=await recordAnswer({session,item,correct:true,answer:'',responseTimeMs:1,activeTimeMs:1,timezone:'UTC',now,database:db});
...
describe('передача файла копии',()=>{
 ...
 it('файловый share предпочтителен; отмена отличается от ошибки и не считается сохранением',async()=>{
  expect(await transferFile(blob,'a.json',{canShare:()=>true,share:async()=>undefined,download:()=>undefined,downloadSupported:true})).toBe('shared');
  expect(await transferFile(blob,'a.json',{canShare:()=>true,share:async()=>{throw Object.assign(new Error('cancel'),{name:'AbortError'})},download:()=>undefined,downloadSupported:true})).toBe('cancelled');
  const downloads:string[]=[];
  expect(await transferFile(blob,'a.json',{canShare:()=>true,share:async()=>{throw new Error('fail')},download:(_,name)=>{downloads.push(name)},downloadSupported:true})).toBe('downloaded');
  expect(downloads).toEqual(['a.json']);
  expect(await transferFile(blob,'a.json',{canShare:()=>false,download:()=>{downloads.push('x')},downloadSupported:true})).toBe('downloaded');
  expect(await transferFile(blob,'a.json',{downloadSupported:false})).toBe('unsupported');
  expect(await transferFile(blob,'a.json',{canShare:()=>true,share:async()=>{throw new Error('fail')},downloadSupported:false})).toBe('failed');
 ...
 it('копия профиля Telegram читается обычным восстановлением и не переносит служебные ключи синхронизации',async()=>{
  ...
  await telegram.delete();await telegram.open();await installLessons(telegram,['lesson-1-1']);
