...
test('«Учить курс» ставит все уроки курса, а новый урок подхватывается при следующем запуске',async({page})=>{
 ...
 await expect.poll(installed).toEqual(['lesson-1-1','lesson-1-1-extra','lesson-1-2','lesson-1-3','lesson-1-4','lesson-2-1','lesson-2-2','lesson-2-3']);

 // Урок опубликован: подписанный курс доустанавливает его сам, без нажатий.
 await page.unroute('**/content/catalog.json');
 await page.goto('/');
 await ready(page);
 await expect.poll(installed,{timeout:20000}).toEqual(['lesson-1-1','lesson-1-1-extra','lesson-1-2','lesson-1-3','lesson-1-4','lesson-2-1','lesson-2-2','lesson-2-3','lesson-2-4']);
});
