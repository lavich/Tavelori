 ...
 */
const provenance={sourceLabel:'Существующий пример проекта, иллюстрация формата',locator:'content/words/γράφω.yaml, examples[0]',excerpt:'Γράφω ένα γράμμα.',operation:'cloze-from-source'};
const phrase={text:'Γράφω ένα γράμμα.',translation:'Я пишу письмо.',provenance:{...provenance,operation:'verbatim'}};
const cloze={template:'{{gap}} ένα γράμμα.',answer:'Γράφω',acceptedAnswers:['Γράφω'],context:'Я пишу письмо.',related:{kind:'word',id:'w11-27'},target:{kind:'verb-form',ref:'w11-27',features:{tense:'present',person:1,number:'singular'}},provenance};
const mixed=({phrases={'p-grafo':phrase},clozes={'c-grafo':cloze},lesson,mutate}:MixedFiles={})=>buildMixed({phrases,clozes,lesson:lesson??{title:'Смешанный урок',language:'el',items:[...Object.keys(phrases).map(id=>({kind:'phrase',id})),{kind:'word',id:'w11-27'},...Object.keys(clozes).map(id=>({kind:'cloze',id}))]},mutate});
...
describe('сборка смешанного урока',()=>{
 it('собирает фразы, пропуски и слова в авторском порядке и проходит проверку пакета',()=>{
  ...
  expect(mixedPack.items.map(item=>[item.kind,item.id,item.position])).toEqual([['phrase','p-grafo',0],['word','w11-27',1],['cloze','c-grafo',2]]);
  expect(mixedPack.phrases[0]).toMatchObject({id:'p-grafo',text:'Γράφω ένα γράμμα.',translation:'Я пишу письмо.'});
  expect(mixedPack.clozes[0]).toMatchObject({id:'c-grafo',answer:'Γράφω',target:cloze.target,related:{kind:'word',id:'w11-27'}});
 ...
 it('ревизия меняется от цели и текста, идентификатор — нет',()=>{
  ...
  const retargeted=pack(mixed({clozes:{'c-grafo':{...cloze,target:{...cloze.target,features:{...cloze.target.features,number:'plural'}}}}})).clozes[0];
 ...
 it('отклоняет неверную разметку пропуска, ссылки и происхождение',()=>{
  expect(()=>mixed({clozes:{'c-grafo':{...cloze,template:'Γράφω {{gap}} {{gap}}.'}}})).toThrow(/clozes\/c-grafo.yaml.*ровно один/);
  expect(()=>mixed({clozes:{'c-grafo':{...cloze,answer:''}}})).toThrow(/пуст/);
  expect(()=>mixed({clozes:{'c-grafo':{...cloze,template:'Γρά{{gap}} ένα γράμμα.',answer:'φω',acceptedAnswers:['φω'],provenance:{...provenance,excerpt:undefined}}}})).toThrow(/часть слова/);
  expect(()=>mixed({clozes:{'c-grafo':{...cloze,related:{kind:'word',id:'w99-99'}}}})).toThrow(/w99-99/);
  expect(()=>mixed({lesson:{title:'x',items:[{kind:'cloze',id:'нет'}]}})).toThrow(/пропуска нет нет в clozes/);
  ...
  expect(()=>mixed({clozes:{'c-grafo':{...cloze,provenance:{sourceLabel:'Агент',operation:'requested-generation'}}}})).toThrow(/request/);
  expect(()=>mixed({clozes:{'c-grafo':{...cloze,template:'{{gap}} ένα βιβλίο.'}}})).toThrow(/восстановленное предложение/);
 });
 it('отклоняет цель не в kebab-case и вложенные признаки',()=>{
  ...
  expect(()=>mixed({clozes:{'c-grafo':{...cloze,target:{kind:'verb-form',features:{Tense:'present'}}}}})).toThrow(/kebab-case/);
  expect(()=>mixed({clozes:{'c-grafo':{...cloze,target:{kind:'verb-form',features:{tense:{nested:true}}}}}})).toThrow(/строка, число или да\/нет/);
  expect(pack(mixed({clozes:{'c-grafo':{...cloze,target:{kind:'some-new-kind',features:{'x-y':true}}}}})).clozes[0].target).toEqual({kind:'some-new-kind',features:{'x-y':true}});
 });
 it('отклоняет дубликаты и сирот, разные скрытые места допустимы',()=>{
  expect(()=>mixed({clozes:{'c-grafo':cloze,'c-twin':{...cloze,target:undefined}}})).toThrow(/clozes\/c-twin.yaml повторяет пропуск/);
  expect(()=>mixed({phrases:{'p-grafo':phrase,'p-twin':phrase}})).toThrow(/phrases\/p-twin.yaml повторяет фразу/);
  expect(()=>mixed({clozes:{'c-grafo':cloze,'c-orphan':{...cloze,template:'Γράφω ένα {{gap}}.',answer:'γράμμα',acceptedAnswers:['γράμμα']}},lesson:{title:'x',items:[{kind:'phrase',id:'p-grafo'},{kind:'cloze',id:'c-grafo'}]}})).toThrow(/clozes\/c-orphan.yaml не входит ни в один урок/);
  const two=pack(mixed({clozes:{'c-grafo':cloze,'c-other':{...cloze,template:'Γράφω ένα {{gap}}.',answer:'γράμμα',acceptedAnswers:['γράμμα'],target:undefined}}}));
