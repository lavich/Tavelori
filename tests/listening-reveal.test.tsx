...
const wordItem=(over:Partial<Word>={}):SessionItem=>({id:'i1',ref:{kind:'word',id:'w1'},unitKey:'word:w1',
 card:{kind:'word',word:word(over)},type:'listening',
 options:['το σπίτι','η πόρτα','το νερό','ο δρόμος'],isNew:false,mode:'scheduled',expectedVersion:1});
const phraseItem=(over:Partial<Phrase>={}):SessionItem=>({id:'i2',ref:{kind:'phrase',id:'p1'},unitKey:'phrase:p1',
 card:{kind:'phrase',phrase:phrase(over)},type:'listening',
 options:['Καλημέρα','Καληνύχτα','Ευχαριστώ','Παρακαλώ'],isNew:false,mode:'scheduled',expectedVersion:1});
...
const show=async(item:SessionItem)=>{
 ...
 await act(async()=>{root!.render(<Listening item={item} onAnswer={async()=>true} onNext={()=>undefined}/>)});
 return container;
};
const press=async(element:Element|undefined)=>{await act(async()=>{(element as HTMLElement).click()})};
const option=(host:HTMLElement,text:string)=>Array.from(host.querySelectorAll('[data-testid="option"]')).find(button=>button.textContent?.includes(text));
const dontKnow=(host:HTMLElement)=>Array.from(host.querySelectorAll('button')).find(button=>button.textContent==='Не знаю');
