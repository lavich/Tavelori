...
const snapshot=(size=0):CompactSnapshot=>({
 ...
 states:Array.from({length:size},(_,index)=>({ref:{kind:'word' as const,id:`w11-${String(index).padStart(2,'0')}`},card:{due:'2026-09-17T00:00:00.000Z',stability:2.123456789012,difficulty:5.987654321098,elapsed_days:1,scheduled_days:1,reps:1,lapses:0,state:2,learning_steps:0},introducedAt:'2026-09-15T00:00:00.000Z',version:1})),
 skills:[],stats:{days:[],recentByType:{},answers:0,answeredKeys:[]},
});
const meta=(id:string,device:string,clock:Record<string,number>,resolves:string[]=[]):Omit<VersionMeta,'parts'|'checksum'>=>({id,device,clock,createdAt:'2026-09-16T10:00:00.000Z',format:SNAPSHOT_FORMAT,resolves});
...
function fakeCloudStorage(memory=memoryTransport()):TelegramCloudStorage{
 const call=<T,>(task:Promise<T>,done?:(error:string|null,value?:T)=>void)=>{task.then(value=>done?.(null,value),error=>done?.(String(error.message)))};
 return {
  setItem:(key,value,done)=>call(memory.setItem(key,value).then(()=>true),done),
  getItem:(key,done)=>call(memory.getItems([key]).then(items=>items[key]),done),
  getItems:(keys,done)=>call(memory.getItems(keys),done),
  removeItem:(key,done)=>call(memory.removeItems([key]).then(()=>true),done),
  removeItems:(keys,done)=>call(memory.removeItems(keys).then(()=>true),done),
  getKeys:done=>call(memory.getKeys(),done),
 };
}
const variants=[
 ['память',()=>{const memory=memoryTransport();return {transport:memory,memory}}],
 ['CloudStorage',()=>{const memory=memoryTransport();return {transport:cloudStorageTransport(fakeCloudStorage(memory)),memory}}],
] as const;
...
describe.each(variants)('контракт адаптера синхронизации: %s',(_,make)=>{
 it('публикует версию частями с указателем последним и читает её целиком',async()=>{
  ...
  expect(writes.slice(0,-1)).toEqual(Array.from({length:published.parts},(_,index)=>partKey('dev1-1',index)));
 ...
 it('обрыв на любой стадии не выдаёт неполную версию за доступную',async()=>{
  ...
  for(let failAt=0;failAt<=total;failAt++){
   const memory=memoryTransport({intercept:(op,key)=>{if(op==='setItem'&&memory.log.filter(entry=>entry.startsWith('setItem')).length===failAt+1)throw new SyncError('transport',`обрыв на ${key}`)}});
   const adapter=kvAdapter(memory);
   await expect(adapter.publishVersion(meta('dev1-1','dev1',{dev1:1}),data)).rejects.toMatchObject({kind:'transport'});
   const listing=await adapter.listPointers();
 ...
 it('неизвестный формат не читается и не считается пригодной версией',async()=>{
  ...
  await expect(adapter.readVersion(listing.pointers[0])).rejects.toMatchObject({kind:'format',message:expect.stringMatching(/Обновите приложение/)});
 ...
 it('лимит ключей проверяется до записи с учётом собственного указателя',async()=>{
  ...
  await expect(adapter.publishVersion(meta('dev1-2','dev1',{dev1:2}),data)).rejects.toMatchObject({kind:'limit'});
...
it('отключённый транспорт веба отвечает явной ошибкой, а не фиктивным успехом',async()=>{
 ...
 await expect(adapter.publishVersion(meta('a-1','a',{a:1}),snapshot(1))).rejects.toMatchObject({kind:'unavailable'});
});
it('доменные модули не импортируют Telegram API',async()=>{
 const {readFileSync,readdirSync}=await import('node:fs');
 const files=[...readdirSync('src/domain').map(file=>`src/domain/${file}`),...readdirSync('src/storage').map(file=>`src/storage/${file}`),'src/sync/adapter.ts','src/sync/coordinator.ts','src/sync/snapshot.ts'];
