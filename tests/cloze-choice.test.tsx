...
const cloze:Cloze={id:'c1',template:'{{gap}} ένα γράμμα.',answer:'Γράφω',acceptedAnswers:['Γράφω'],
 context:'Я пишу письмо.',explanation:'Первое лицо настоящего времени.',
 provenance:{sourceLabel:'тест',operation:'cloze-from-source'},createdAt:'',updatedAt:''};
const item=(options:string[]):SessionItem=>({id:'i1',ref:{kind:'cloze',id:'c1'},unitKey:'["cloze","c1"]',
 card:{kind:'cloze',cloze},type:'cloze',options,isNew:false,mode:'practice',expectedVersion:1});
...
const show=async(entry:SessionItem)=>{
 ...
 await act(async()=>{root!.render(<ClozeExercise item={entry} onAnswer={async()=>true} onNext={()=>undefined}/>)});
 return container;
};
const press=async(element:Element|undefined)=>{await act(async()=>{(element as HTMLElement).click()})};
const choice=(host:HTMLElement,text:string)=>Array.from(host.querySelectorAll('[data-testid="cloze-option"]')).find(button=>button.textContent===text);
...
describe('пропуск с вариантами ответа',()=>{
 ...
 it('верный вариант раскрывает предложение и объяснение',async()=>{
  ...
  expect(host.querySelector('[data-testid="cloze-explanation"]')!.textContent).toBe('Первое лицо настоящего времени.');
