...
import {buildMixed, MIXED_CLOZES, MIXED_LESSON, MIXED_PHRASES, mixedContent, mixedPackage} from './helpers/mixed-fixture';
...
describe('шаблоны инструкции',()=>{
 it('слово, фраза, пропуск и смешанный урок из docs/ проходят тот же валидатор, что и каталог',()=>{
  ...
  try{
   ...
   expect(pack.items.map(item=>[item.kind,item.id])).toEqual([['phrase','p-example'],['word','w-example'],['cloze','c-example']]);
   expect(pack.phrases[0]).toMatchObject({id:'p-example',text:'Το δείγμα είναι απλό.',translation:'Образец простой.'});
   expect(pack.clozes[0]).toMatchObject({id:'c-example',answer:'είναι',target:{kind:'verb-form',ref:'w-example',features:{tense:'present',person:3,number:'singular'}}});
   expect(pack.clozes[0].provenance.parts?.target).toMatchObject({operation:'requested-transform',request:expect.any(String)});
 ...
 it('инструкция ссылается на все шаблоны и на навык, навык — на инструкцию',()=>{
  ...
  for(const name of ['word','phrase','cloze','lesson'])expect(doc,name).toContain(`lesson-authoring/${name}.yaml`);
  expect(doc).toContain('prepare-lesson/SKILL.md');
  // Границы, которые инструкция обязана называть явно.
  for(const rule of ['Только присланное и запрошенное','`extract`','`transform`','`generate`','kebab-case','requested-generation','provenance.parts'])
...
describe('прогон инструкции на существующем материале проекта',()=>{
 ...
 it('повторная обработка сохраняет идентификаторы: опечатка и поздняя цель меняют только ревизию',()=>{
  ...
  const typo={...(MIXED_CLOZES['c-vouno'] as Record<string,unknown>),explanation:'Прилагательное согласуется с существительным среднего рода в числе.'};
  const late={...(MIXED_CLOZES['c-gramma'] as Record<string,unknown>),target:{kind:'noun-form',features:{case:'accusative'}},
   provenance:{...(MIXED_CLOZES['c-gramma'] as {provenance:Record<string,unknown>}).provenance,parts:{target:{sourceLabel:'Разметка по запросу',operation:'requested-transform',request:'Отметить, что тренирует карточка'}}}};
  const again=buildMixed({phrases:MIXED_PHRASES,clozes:{...MIXED_CLOZES,'c-vouno':typo,'c-gramma':late},
  ...
  const pair=(id:string)=>[before.clozes.find(item=>item.id===id)!,after.clozes.find(item=>item.id===id)!] as const;
  ...
  expect(after.phrases.map(item=>[item.id,item.revision])).toEqual(before.phrases.map(item=>[item.id,item.revision]));
