...
import {ContentError, parseCatalog, parsePackage, SCHEMA_VERSION, SUPPORTED_SCHEMAS, type ContentPackage} from '../src/content/schema';
import {content} from './helpers/content';

const provenance={sourceLabel:'Иллюстрация формата',locator:'fixture',excerpt:'Καλημέρα.',operation:'verbatim' as const};
const word=content.packages[0].words[0];
const phrase={id:'p-1',text:'Καλημέρα.',translation:'Доброе утро.',provenance,revision:'r1'};
const mixed:Record<string,unknown>={
 schemaVersion:3,id:'mixed',courseId:'c',version:'v1',language:'el',lesson:{title:'Смешанный'},
 words:[word],phrases:[phrase],
 items:[{kind:'phrase',id:'p-1',position:0},{kind:'word',id:word.id,position:1}],
...
describe('пакет схемы 3',()=>{
 ...
 it('смешанный пакет проходит проверку и сохраняет порядок карточек',()=>{
  ...
  expect(pack.items.map(item=>[item.kind,item.id])).toEqual([['phrase','p-1'],['word',word.id]]);
 ...
 it('пакет без слов допустим при наличии других карточек',()=>{
  const pack=parsePackage({...mixed,words:[],media:[],items:[{kind:'phrase',id:'p-1',position:0}]});
 ...
 it('отклоняет неверные ссылки, дубликаты и вид карточки',()=>{
  expect(()=>parsePackage({...mixed,items:[...(mixed.items as unknown[]),{kind:'phrase',id:'нет',position:3}]})).toThrow(/которой нет в пакете/);
 ...
 it('требует происхождение с операцией и запрос для преобразования и генерации',()=>{
  const bad=(patch:Record<string,string|undefined>)=>()=>parsePackage({...mixed,phrases:[{...phrase,provenance:{...provenance,...patch}}]});
  expect(()=>parsePackage({...mixed,phrases:[{...phrase,provenance:undefined}]})).toThrow(/provenance/);
  ...
  expect(parsePackage({...mixed,phrases:[{...phrase,provenance:{sourceLabel:'Агент по запросу',operation:'requested-generation',request:'составь пример'}}]}).phrases[0].provenance.operation).toBe('requested-generation');
 });
});
...
describe('совместимость со схемой 2',()=>{
 ...
 const legacy=()=>{
  const {phrases:_p,items,...rest}=v2 as unknown as Record<string,unknown>&{items:{kind:string;id:string;position:number}[]};
 ...
 it('каталог схемы 2 читается с нулевым счётчиком фраз',()=>{
  ...
  const old={...raw,schemaVersion:2,lessons:raw.lessons.map(({phraseCount:_a,cardCount:_b,...entry}:Record<string,unknown>)=>entry)};
  const catalog=parseCatalog(old);
  expect(catalog.lessons[0]).toMatchObject({phraseCount:0,cardCount:catalog.lessons[0].wordCount});
 });
