 ...
 * Курс получает дополнительный урок `lesson-mixed`; прежние уроки собираются без изменений.
 */
...
export const MIXED_ITEMS=[
 {kind:'phrase',id:'p-grafo'},{kind:'word',id:'w11-27'},
 {kind:'phrase',id:'p-vouno'},{kind:'phrase',id:'p-paidi'},
 {kind:'phrase',id:'p-anoixi'},{kind:'phrase',id:'p-silent'},
];
export const MIXED_LESSON='lesson-mixed';

export interface MixedFiles {phrases?:Record<string,unknown>;lesson?:Record<string,unknown>;mutate?:(root:string)=>void}
...
export function buildMixed({phrases=MIXED_PHRASES,lesson,mutate}:MixedFiles={}):BuiltContent{
 ...
 for(const dir of ['words','lessons','art','courses','phrases','audio']) if(existsSync(join('content',dir)))cpSync(join('content',dir),join(root,dir),{recursive:true});
 ...
 const items=lesson?undefined:[...Object.keys(phrases).map(id=>({kind:'phrase',id})),{kind:'word',id:'w11-27'}];
