 ...
 * Экраны за пределами «Сегодня» загружаются по маршруту (`React.lazy` в `src/app/App.tsx`) и сюда не входят.
 ...
 * 902 808 перед разделением маршрутов. После разделения граф — 653 905 байт (стартовый чанк из них 329 075).
...
if(!entryPath)problems.push('в index.html не найден стартовый модуль');
 ...
 console.log(`стартовый граф ${initialPaths.length} чанк(ов): ${size} байт (предел ${INITIAL_LIMIT}); из них ${entryPath}: ${Buffer.byteLength(entry)}`);
 ...
 if(withSdk.length)problems.push(`SDK отчётов о сбоях попал в стартовый граф: ${withSdk.map(chunk=>chunk.path).join(', ')}`);
 const sdkChunks=files.filter(file=>file.endsWith('.js')&&!initialPaths.some(path=>file===join(dist,path))&&readFileSync(file,'utf8').includes(SDK_MARKER));
